...
/*
 * Удаление студента из базы данных по ID
 * Параметры:
 *   students - указатель на указатель массива студентов (для изменения массива)
 *   count    - указатель на счётчик студентов
 * Возвращает: 0 при успехе, 1 при ошибке
 */
int del_student(Student** students, int* count) {
    ...
    // Запрос ID студента для удаления
    int id;
    printf("\n┌─ Удаление студента ───────────┐\n");
    printf("│ Введите ID студента: ");
    scanf("%d", &id);

    // Поиск индекса студента с заданным ID
    int found_index = -1;  // -1 означает "не найдено"
    for(int i = 0; i < *count; i++) {
        if(id == (*students)[i].id) {
            found_index = i;  // Сохраняем индекс найденного студента
            break;
        }
    }
    ...
    if(found_index == -1) {
        printf("└───────────────────────────────┘\n");
        printf("\n✖ Студент с ID %d не найден!\n", id);
        return 1;
    }

    // Выводим информацию об удаляемом студенте
    printf("│ Найден студент:\n");
    printf("│   ID: %d\n", (*students)[found_index].id);
    printf("│   Имя: %s\n", (*students)[found_index].name);
    printf("│   Возраст: %d\n", (*students)[found_index].age);
    printf("│   Оценка: %.2f\n", (*students)[found_index].grade);
    printf("└───────────────────────────────┘\n");

    // Сдвигаем все элементы после удаляемого на одну позицию влево
    for (int i = found_index; i < *count - 1; i++){
        (*students)[i] = (*students)[i + 1];  // Перезаписываем текущий элемент следующим
    }

    // Уменьшаем количество студентов
    (*count)--;

    // Уменьшаем размер массива через realloc
    if(*count > 0) {
        // Если остались студенты - уменьшаем массив
        Student* temp = realloc(*students, *count * sizeof(Student));
        if(!temp) {
            printf("\n✖ Ошибка: не удалось перераспределить память!\n");
            return 1;
        }
        *students = temp;  // Обновляем указатель на новый массив
    } else {
        // Если удалили последнего студента - освобождаем всю память
        free(*students);
        *students = NULL;
    }
